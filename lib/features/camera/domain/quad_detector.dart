...
class Quad {
  ...
  /// 90° clockwise in normalized [0,1]². Matches [Raster.rotatedClockwise]:
  /// the old bottom-left corner becomes the new top-left.
  Quad rotatedClockwise() {
  ...
  /// True when this quad is a page photographed inside the frame, not a scan
  /// that already fills the image.
  ///
  /// Used by the in-app camera path and by Detect edges. Native scans must
  /// not auto-apply an inset quad — printed text on an already-cropped page
  /// matches this heuristic and would throw away half the sheet.
  bool get isClearlyInset {
    if (areaRatio < 0.05 || areaRatio > 0.88) return false;
    const edge = 0.04;
    var insetCorners = 0;
