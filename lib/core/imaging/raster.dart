...
class Raster {
  ...
  ///
  /// Images with alpha are composited onto white first. PDF rasters use a
  /// transparent background; dropping the alpha channel would save them as
  /// a black page.
  static Raster fromImage(img.Image image) {
    final flat = flattenOntoWhite(image);
    final bytes = flat.getBytes(order: img.ChannelOrder.rgb);
    return Raster.fromPixels(flat.width, flat.height, bytes);
  }
  ...
  /// 90° clockwise. New size is `height × width`.
  Raster rotatedClockwise() {
    final newWidth = height;
    final newHeight = width;
    final dst = Uint8List(pixels.length);
    for (var y = 0; y < height; y++) {
      for (var x = 0; x < width; x++) {
        final src = (y * width + x) * 3;
        // (x, y) → (height - 1 - y, x)
        final di = (x * newWidth + (height - 1 - y)) * 3;
        dst[di] = pixels[src];
        dst[di + 1] = pixels[src + 1];
        dst[di + 2] = pixels[src + 2];
      }
    }
    return Raster.fromPixels(newWidth, newHeight, dst);
  }

  /// Rotate 90° clockwise [turns] times (0–3 after wrapping).
  Raster rotatedTurns(int turns) {
    var out = this;
    var n = turns % 4;
    if (n < 0) n += 4;
    for (var i = 0; i < n; i++) {
      out = out.rotatedClockwise();
    }
    return out;
  }
