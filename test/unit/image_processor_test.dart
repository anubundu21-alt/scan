...
/// A cream/warm-lit notebook page: neutral-ish ink on paper that is warmer in
/// red than in blue, with a lighting falloff across it. This is the case that
/// used to come back orange-brown.
Raster _creamPage({int width = 120, int height = 90}) {
  final raster = Raster(width, height);
  for (var y = 0; y < height; y++) {
    for (var x = 0; x < width; x++) {
      // Lighting falls off toward the right, as a phone shadow does.
      final fall = 1.0 - (x / width) * 0.82;
      // Warm paper: red high, blue noticeably lower.
      var r = (247 * fall).round();
      var g = (236 * fall).round();
      var b = (206 * fall).round();
      // A little sensor noise, which is what runaway gain in the shadow
      // amplifies into visible grey-brown mottling.
      final noise = ((x * 7 + y * 13) % 5) - 2;
      r += noise;
      g += noise;
      b += noise;
      // Ruled lines and ink every 10th column.
      if (x % 10 == 3 || x % 10 == 4) {
        r = (r * 0.35).round();
        g = (g * 0.35).round();
        b = (b * 0.35).round();
...
/// Mean R, G, B over the pixels the eye reads as paper (the bright ones).
List<double> _paperMeans(Raster raster) {
  final luma = raster.toLuma();
  var sumR = 0, sumG = 0, sumB = 0, n = 0;
  for (var p = 0; p < luma.length; p++) {
    if (luma[p] < 140) continue; // skip ink
    final i = p * 3;
    sumR += raster.pixels[i];
    sumG += raster.pixels[i + 1];
    sumB += raster.pixels[i + 2];
    n++;
  }
...
void main() {
  ...
  group('filter kernels', () {
    test('Auto leaves cream paper neutral rather than orange', () {
      ...
      // Precondition: the source really is warm — red well above blue.
      expect(before[0] - before[2], greaterThan(30));

      final out = applyAdjustments(
        page,
        const ScanAdjustments(filter: ScanFilter.magic),
      );
      final after = _paperMeans(out);

      // The cast is gone: the channels land close together.
      expect(
        (after[0] - after[2]).abs(),
        lessThan(12),
        reason:
      ...
      expect(after[1], greaterThan(215));
    });
