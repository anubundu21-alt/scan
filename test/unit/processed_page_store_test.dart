...
void main() {
  ...
  test('a PDF import is marked so Sign PDF can list it', () async {
    final store = newStore();
    await store.createProcessedDocument(
      pages: [processed(capture('letter.pdf.png', 3), 90)],
      title: 'Offer letter',
    ...
    final pdf = reopened.singleWhere((d) => d.title == 'Offer letter');
    expect(pdf.importedFromPdf, isTrue);
    expect(pdf.isPdfForSigning, isTrue);
    expect(
      reopened.where((d) => !d.isPdfForSigning),
      isNotEmpty,
      reason: 'scans must not appear as PDFs',
    );
  });
  ...
  test('crop and filter settings survive a restart', () async {
    ...
    expect(page.adjustments.rotationTurns, 1);
