...
The Dart package name is still `scan2` (repo and imports). The store name,
bundle id, and Android application id are Scanella / `com.scanella.mobile`.

Point the camera at a page: Scanella tracks the edges live, releases the shutter
...
See `android/README.md`. GitHub Actions → "Android Release (Play)" builds a
signed AAB for `com.scanella.mobile`. Check the upload box only when the Play
Console secrets are set.
